...
import za.co.absa.hyperdrive.trigger.models.{Event, JobInstance, SensorProperties}
...
class EventProcessor(eventRepository: EventRepository, jobDefinitionsRepository: JobDefinitionsRepository, jobInstanceRepository: JobInstanceRepository) {
  ...
  def eventProcessor(events: Seq[Event], sensorProperties: SensorProperties)(implicit ec: ExecutionContext): Future[Boolean] = {
    val fut = processEvents(events, sensorProperties)
    logger.debug(s"Processing events. Sensor id: ${sensorProperties.sensorId}. Events: ${events.map(_.id)}" )
    fut
  }

  private def processEvents(events: Seq[Event], sensorProperties: SensorProperties)(implicit ec: ExecutionContext): Future[Boolean] = {
    eventRepository.getExistEvents(events.map(_.sensorEventId)).flatMap { eventsIdsInDB =>
      ...
      jobDefinitionsRepository.getJobDefinition(sensorProperties.sensorId).flatMap {
