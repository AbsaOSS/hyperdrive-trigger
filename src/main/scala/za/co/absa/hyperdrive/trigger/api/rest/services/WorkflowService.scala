...
class WorkflowServiceImpl(
  ...
  private def mergeWithOriginalWorkflow(originalWorkflow: WorkflowJoined, workflow: WorkflowJoined) =
    workflow.copy(
      ...
      dagDefinitionJoined = workflow.dagDefinitionJoined
        .copy(id = originalWorkflow.dagDefinitionJoined.id, workflowId = originalWorkflow.id)
    )
    ...
    workflowIds.distinct.length match {
      ...
      case numberOfWorkflows if numberOfWorkflows > generalConfig.maximumNumberOfWorkflowsInBulkRun =>
        throw new ApiException(
          GenericError(s"Cannot trigger more than ${generalConfig.maximumNumberOfWorkflowsInBulkRun} workflows!")
        )
      case _ =>
  ...
  override def runWorkflowJobs(workflowId: Long, jobIds: Seq[Long])(implicit ec: ExecutionContext): Future[Boolean] = {
    ...
    workflowRepository
      ...
      .flatMap { joinedWorkflow =>
        val dagDefinitionJoined = joinedWorkflow.dagDefinitionJoined

        val anyJobIdIsNotPartOfWorkflow =
          !jobIds.forall(id => dagDefinitionJoined.jobDefinitions.map(_.id).contains(id))

        if (anyJobIdIsNotPartOfWorkflow) {
          Future.successful(false)
        } else {
          val dagDefinitionWithFilteredJobs = dagDefinitionJoined
            .copy(jobDefinitions = dagDefinitionJoined.jobDefinitions.filter(job => jobIds.contains(job.id)))
          for {
  ...
  override def exportWorkflows(
    workflowIds: Seq[Long]
  )(implicit ec: ExecutionContext): Future[Seq[WorkflowImportExportWrapper]] =
    for {
  ...
  override def importWorkflows(
    workflowImports: Seq[WorkflowImportExportWrapper]
  )(implicit ec: ExecutionContext): Future[Seq[WorkflowJoined]] = {
    val userName = getUserName.apply()
    for {
  ...
  override def convertToWorkflowJoined(
    workflowImport: WorkflowImportExportWrapper
  )(implicit ec: ExecutionContext): Future[WorkflowJoined] =
    convertToWorkflowJoineds(Seq(workflowImport)).transform(
      workflowJoineds =>
  ...
  private def convertToWorkflowJoineds(
    workflowImports: Seq[WorkflowImportExportWrapper]
  )(implicit ec: ExecutionContext): Future[Seq[WorkflowJoined]] =
    resolveJobTemplates(workflowImports)
      .map(resetSchedulerInstanceId)

  private def resolveJobTemplates(
    workflowImports: Seq[WorkflowImportExportWrapper]
  )(implicit ec: ExecutionContext): Future[Seq[WorkflowJoined]] = {
    val jobTemplatesNames = workflowImports.flatMap(_.jobTemplates.map(_.name)).distinct
    jobTemplateService.getJobTemplateIdsByNames(jobTemplatesNames).flatMap { newNameIdMap =>
      val workflowJoinedsEit = workflowImports.map { workflowImport =>
        ...
        if (missingTemplates.nonEmpty) {
          Left(
            BulkOperationError(
              workflowImport.workflowJoined,
              GenericError(
                s"The following Job Templates don't exist yet and have to be created before importing" +
          ...
          def getNewId(oldTemplateId: Long) = {
            val name = oldIdNameMap.get(oldTemplateId) match {
              case Some(x) => x
              case None =>
                throw new IllegalArgumentException(s"Template Id $oldTemplateId is not referenced in $oldIdNameMap")
            }
          ...
          val workflowWithResolvedJobTemplateIds = workflowImport.workflowJoined
            .copy(dagDefinitionJoined =
              workflowImport.workflowJoined.dagDefinitionJoined
                .copy(jobDefinitions =
                  workflowImport.workflowJoined.dagDefinitionJoined.jobDefinitions
                    .map {
