...
class LiquibaseConfiguration(properties: LiquibaseProperties) extends SpringLiquibase with Repository {
  ...
  override def afterPropertiesSet(): Unit = {
    if (!skipLiquibase) {
      ...
      repositoryLogger.info("Skipping Liquibase")
    }
  }

  private def configureLiquibase(): Unit = {
    repositoryLogger.info("Configuring Liquibase")
    ...
    try {
      ...
      liquibaseOpt match {
        ...
        case None => repositoryLogger.error("Could not configure liquibase")
      }
    } finally {
  ...
  private def updateMigrations(liquibase: Liquibase): Unit = {
    ...
    if (!unrunChangeSets.isEmpty) {
      ...
      repositoryLogger.debug("Database schema is up-to-date")
    }
  }
