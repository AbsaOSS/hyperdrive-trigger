...
import za.co.absa.hyperdrive.trigger.api.rest.services.{
  DagInstanceService,
  DagInstanceServiceImpl,
  JobTemplateFixture,
  JobTemplateResolutionServiceImpl,
  JobTemplateService,
  JobTemplateServiceImpl,
  JobTemplateValidationServiceImpl
}
...
class TimeSensorIntegrationPostgresTest
    extends FlatSpec
    with Matchers
  ...
  private val jobTemplateHistoryRepository: JobTemplateHistoryRepositoryImpl = new JobTemplateHistoryRepositoryImpl(
    dbProvider
  )
  private val jobTemplateRepository: JobTemplateRepositoryImpl =
  ...
  private val jobTemplateValidationService: JobTemplateValidationServiceImpl = new JobTemplateValidationServiceImpl(
    jobTemplateRepository
  )
  private val jobTemplateService: JobTemplateService =
  ...
  it should "persist an event when the time sensor is fired" in {
    val processor =
      new EventProcessor(eventRepository, dagDefinitionRepository, dagInstanceRepository, dagInstanceService)
    ...
    val jobParameters1 = SparkDefinitionParameters(
      ...
      jobJar = Option("spark-job.jar"),
      mainClass = Option("TheMainClass")
    )
    val jobDefinition1 = JobDefinition(-1L, Some(sparkTemplateId), "Time-Sensor Job 1", jobParameters1, 1)
    val jobParameters2 = SparkDefinitionParameters(
      jobType = JobTypes.Spark,
      jobJar = Option("spark-job-2.jar"),
      mainClass = Option("TheMainClass")
    )
