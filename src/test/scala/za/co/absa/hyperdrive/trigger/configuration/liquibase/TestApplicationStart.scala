...
class TestApplicationStart extends FlatSpec with Matchers with SpringIntegrationTest {

  @Inject() var hyperDriverManager: HyperDriverManager = _
  @Inject() var workflowRepository: WorkflowRepository = _

  it should "start the application, including sql migrations" in {
    hyperDriverManager.isManagerRunning shouldBe true
    await(workflowRepository.getWorkflows()) shouldBe empty
  }
}
