...
class SearchableTableQueryTest
    extends FlatSpec
    with Matchers
    with BeforeAndAfterAll
    with BeforeAndAfterEach
    with TestSearchableRepositoryTestBase {
  ...
  it should "add all filters to the query" in {
    ...
    val dateTimeRangeFilterSeq = Some(
      Seq(
    ...
    val equalsMultipleFilterSeq =
      Some(Seq(EqualsMultipleFilterAttributes(field = stringField3, values = List("bar", "value", "str"))))
  ...
  "the date-time-range filter" should "find values within the inclusive range" in {
    ...
    val filter = DateTimeRangeFilterAttributes(
      field = TestSearchableTableFieldNames.localDateTimeField,
      start = startDate,
      end = endDate
    )
    val searchRequest =
      TableSearchRequest(dateTimeRangeFilterAttributes = Some(Seq(filter)), sort = None, from = 0, size = 50)
  ...
  it should "return an empty list if start is higher than end" in {
    ...
    val filter = search.DateTimeRangeFilterAttributes(
      field = TestSearchableTableFieldNames.localDateTimeField,
  ...
  "the int-range filter" should "find values within the inclusive range" in {
    val filter =
      IntRangeFilterAttributes(field = TestSearchableTableFieldNames.longField, start = Option(0), end = Option(2))
    val searchRequest =
      TableSearchRequest(intRangeFilterAttributes = Some(Seq(filter)), sort = None, from = 0, size = 50)
  ...
  "the multiple-attributes filter" should "find values exactly equal to the list of strings" in {
    val filter =
      EqualsMultipleFilterAttributes(field = TestSearchableTableFieldNames.stringField3, values = List("foo", "bar"))
    val searchRequest =
      TableSearchRequest(equalsMultipleFilterAttributes = Some(Seq(filter)), sort = None, from = 0, size = 50)

    val result = await(db.run(underTest.search(searchRequest)))

    val expected =
      TestSearchableData.testSearchableEntities.filter(data => data.stringValue3 == "foo" || data.stringValue3 == "bar")
  ...
  it should "not find values that do not contain the search string" in {
    val filter =
      EqualsMultipleFilterAttributes(field = TestSearchableTableFieldNames.stringField3, values = List("fo", "ba"))
    val searchRequest =
      TableSearchRequest(equalsMultipleFilterAttributes = Some(Seq(filter)), sort = None, from = 0, size = 50)
  ...
  "the int-range filter" should "find values less or equal to the right bound" in {
    val filter =
      IntRangeFilterAttributes(field = TestSearchableTableFieldNames.longField, start = None, end = Option(2))
    val searchRequest =
      TableSearchRequest(intRangeFilterAttributes = Some(Seq(filter)), sort = None, from = 0, size = 50)
  ...
  it should "return an empty list if start is higher than end" in {
    val filter =
      IntRangeFilterAttributes(field = TestSearchableTableFieldNames.longField, start = Option(10), end = Option(0))
    val searchRequest =
      TableSearchRequest(intRangeFilterAttributes = Some(Seq(filter)), sort = None, from = 0, size = 50)
  ...
  "the boolean filter" should "find values that are true" in {
    val filter = BooleanFilterAttributes(
      field = TestSearchableTableFieldNames.booleanField,
      value = BooleanValues(isTrue = true, isFalse = false)
    )
    val searchRequest =
      TableSearchRequest(booleanFilterAttributes = Some(Seq(filter)), sort = None, from = 0, size = 50)
  ...
  it should "find values that are false" in {
    val filter = BooleanFilterAttributes(
      field = TestSearchableTableFieldNames.booleanField,
      value = BooleanValues(isTrue = false, isFalse = true)
    )
    val searchRequest =
      TableSearchRequest(booleanFilterAttributes = Some(Seq(filter)), sort = None, from = 0, size = 50)
  ...
  it should "find all values if the filter has checked both true/false" in {
    val filter = BooleanFilterAttributes(
      field = TestSearchableTableFieldNames.booleanField,
      value = BooleanValues(isTrue = true, isFalse = true)
    )
    val searchRequest =
      TableSearchRequest(booleanFilterAttributes = Some(Seq(filter)), sort = None, from = 0, size = 50)
  ...
  it should "find all values if the filter has checked neither true nor false" in {
    val filter = BooleanFilterAttributes(
      field = TestSearchableTableFieldNames.booleanField,
      value = BooleanValues(isTrue = false, isFalse = false)
    )
    val searchRequest =
      TableSearchRequest(booleanFilterAttributes = Some(Seq(filter)), sort = None, from = 0, size = 50)
  ...
  "sort" should "apply a default sort if no sort column is specified" in {
    val searchRequest = TableSearchRequest(sort = None, from = 0, size = 50)
  ...
  it should "apply a sort on the specified column" in {
    ...
    result.total shouldBe TestSearchableData.testSearchableEntities.size
    result.items.size should be > 0
    result.items should contain theSameElementsInOrderAs TestSearchableData.testSearchableEntities.sortBy(
      _.stringValue
    )(Ordering[String].reverse)
  }

  "from" should "drop the first n elements" in {
    val searchRequest = TableSearchRequest(sort = None, from = 1, size = 50)

    val result = await(db.run(underTest.search(searchRequest)))

    result.total shouldBe TestSearchableData.testSearchableEntities.size
    result.items.size should be > 0
    result.items should contain theSameElementsInOrderAs TestSearchableData.testSearchableEntities
      .sortBy(_.longValue)
      .drop(1)
  }

  "size" should "limit the number of elements" in {
    val searchRequest = TableSearchRequest(sort = None, from = 0, size = 2)
