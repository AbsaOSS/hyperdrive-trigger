...
trait RepositoryTestBase extends Repository {
  ...
  object TestData {
    ...
    val notificationRules = Seq(
      NotificationRule(Some("project"), Some("ABC XYZ"), None, Seq(DagInstanceStatuses.Failed), Seq("abc@xyz.com", "def@xyz.com"), updated = None, id = 11L)
    )
  }
